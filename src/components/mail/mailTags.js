  ...
  const renderSingleTag = (tag, key) => {
    ...
    return (
      <li
        key={`tag${key}`}
        onClick={onClick}
        className={`nnaMailTag ${activeClass}`}
      >
        <span className="nnaLabelIndicatorColor" style={{ background }} />
        <span>{tag}</span>
      </li>
    );
  };
  return (
    <MailTagsList className="nnaMailTagList">
      <p className="nnaSectionLabel">Label</p>
